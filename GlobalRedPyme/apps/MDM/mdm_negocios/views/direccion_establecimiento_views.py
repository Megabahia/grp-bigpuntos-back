from ..models import DireccionesEstablecimientosNegocios
from ..serializers import DireccionesEstablecimientosNegociosSerializer
...
def direccionesNegocio_list(request, pk):
    """
    Este metodo sirve para listar los negocios de la tabla negocios de la base datos mdm
    @type pk: El campo pk recibe el id del negocio
    @type request: El campo request recibe page, page_size
    @rtype: Devuelve una lista direcciones, caso contrario devuelve el error generado
    """
    ...
    if request.method == 'POST':
        try:
            ...
            query = DireccionesEstablecimientosNegocios.objects.filter(**filters).order_by('-created_at')
            serializer = DireccionesEstablecimientosNegociosSerializer(query[offset:limit], many=True)
...
def direccionesNegocio_findOne(request, pk):
    """
    Este metodo sirve para obtener una direccion de la tabla direcciones de la base datos mdm
    @type pk: El campo pk recibe el id del negocio
...
def direccionesNegocio_create(request):
    """
    Este metodo sirve para crear la direccion en la tabla negocios de la base datos mdm
    @type request: El campo request recibe los campos de la tabla direcciones
...
def direccionesNegocio_update(request, pk):
    """
    Este metodo sirve para actualizar direcciones de la tabla direcciones de la base datos
    @type pk: El campo pk recibe el id del negocio
    @type request: El campo request recibe los campos de la tabla direcciones
...
def direccionesNegocio_delete(request, pk):
    """
    Este metodo sirve para eliminar las direcciones de la tabla direcciones de la base datos mdm
    @type pk: El campo pk recibe el id de la direccion
    @type request: El campo request no recibe nada
    ...
    try:
        ...
        if request.method == 'DELETE':
            serializer = DireccionesEstablecimientosNegociosSerializer(query,
                                                                       data={'state': '0', 'updated_at': str(nowDate)},
                                                                       partial=True)
            if serializer.is_valid():
                serializer.save()
                createLog(logModel, serializer.data, logTransaccion)
