...
class GRPPYMESRouter:
    """
    A router to control all database operations on models in the
    auth and contenttypes applications.
    """
    route_app_labels = {'pymes_empresas'}


    def db_for_read(self, model, **hints):
        """
        Attempts to read auth and contenttypes models go to grp_pymes_db.
        """
        if model._meta.app_label in self.route_app_labels:
            return 'grp_pymes_db'
        return None
    ...
    def db_for_write(self, model, **hints):
        """
        Attempts to write auth and contenttypes models go to grp_pymes_db.
        """
        if model._meta.app_label in self.route_app_labels:
            return 'grp_pymes_db'
        return None
    ...
    def allow_migrate(self, db, app_label, model_name=None, **hints):
        """
        Make sure the auth and contenttypes apps only appear in the
        'grp_pymes_db' database.
        """
        if app_label in self.route_app_labels:
            return db == 'grp_pymes_db'
        return None
...
class GRPCORPRouter:
    ...
    auth and contenttypes applications.
    """
    route_app_labels = {'corp_cobrarSupermonedas','corp_autorizaciones'}
