    ...
    if request.method == 'POST':
        try:
            ...
            if 'user_id' in request.data:
                ...
                if serializer.is_valid():
                    ...
                    dataEmpresa = {
                        ...
                        'saldo': float(request.data[
                                           'montoSupermonedas']) if monedasEmpresa is None else monedasEmpresa.saldo + float(
            ...
            if referido:
                ...
                monedasReferido = Monedas.objects.filter(empresa_id=None, user_id=str(referidoSerializer['user_id']),
                                                         autorizador_id=None).order_by('-created_at').first()
                catalogoReferencia = Catalogo.objects.filter(nombre='BP_REFERNCIA_COMPARTIDO',
                                                             tipo='GANAR_BP_REFERNCIA').first()
                dataReferido = {
                    ...
                    'saldo': float(catalogoReferencia.valor) if monedasReferido is None else float(
                        monedasReferido.saldo) + float(catalogoReferencia.valor),
