...
def empresas_list_filtro(request):
    ...
    if request.method == 'POST':
        try:
            ...
            # Serializar los datos
            query = Empresas.objects.filter(Q(ruc__icontains=str(request.data["ruc"]), state=1) | Q(
                nombreEmpresa__icontains=str(request.data["ruc"]), state=1)).order_by('-created_at')
...
@api_view(['POST'])
@permission_classes([IsAuthenticated])
def empresas_list_all(request):
    """
    Este metodo sirve para listar todas las empresas de la tabla empresas de la base datos corp
...
def insertarDato_empleado(dato, empresaRuc):
    ...
    """
    try:
        if (empresaRuc != dato[0]):
            return 'No tiene el mismo ruc'
        if ('None' in dato):
        ...
        empresa = Empresas.objects.filter(ruc=dato[0], state=1).first()
...
def empresas_listOne_empresa_empleado(request):
    """
    Este metodo se usa para obtener un empleado de una empresa de la tabla de empleados de la base datos corp
    @type request: El campo request recibe el campo identificacion
    @rtype: DEvuelve el registro del empleado, caso contrario devuelve el error generado
    """
    timezone_now = timezone.localtime(timezone.now())
    logModel = {
