...
def usuario_list(request):
    ...
    try:
        if request.method == 'POST':
            ...
            new_serializer_data = {'cont': usuario.count(), 'info': serializer.data}
            # envio de datos
    ...
    try:
        if request.method == 'POST':
            ...
            if serializer.is_valid():
                ...
                empleado = Empleados.objects.filter(correo=request.data['email']).first()
                if empleado is not None:
                    datosPersona = {
                        ...
                        'nombresCompleto': empleado.nombres + ' ' + empleado.apellidos,
