from ..central_logs.serializers import LogsSerializer
...
def createLog(logModel, logRecibida, logTransaccion):
    try:
        ...
        try:
            permiso = Catalogo.objects.filter(tipo='LOG' + str(logModel['tipo']), nombre=str(logModel['accion']),
                                              state=1).only('valor').first()
