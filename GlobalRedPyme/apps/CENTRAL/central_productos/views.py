...
def productos_list_free_landing(request):
    ...
    if request.method == 'POST':
        try:
            ...
            # Serializar los datos
            query = Productos.objects.filter(**filters).order_by('-created_at')
            serializer = ProductosLandingSerializer(query[:int(limit)], many=True)
            new_serializer_data = {'cont': query.count(),
                                   'info': serializer.data}
            # envio de datos
            return Response(new_serializer_data, status=status.HTTP_200_OK)
...
def productos_create_landing(request):
    ...
    timezone_now = timezone.localtime(timezone.now())
    logModel = {
        'endPoint': logApi + 'create-landing/',
        'modulo': logModulo,
        'tipo': logExcepcion,
        'accion': 'CREAR',
        'fechaInicio': str(timezone_now),
    ...
    if request.method == 'POST':
        try:
            ...
            serializer = ProductosLandingSerializer(data=request.data)
            if serializer.is_valid():
                serializer.save()
                createLog(logModel, serializer.data, logTransaccion)
                return Response(serializer.data, status=status.HTTP_201_CREATED)
...
def productos_listOne_landing(request, pk):
    ...
    try:
        ...
        # tomar el dato
        if request.method == 'GET':
            serializer = ProductosLandingSerializer(query)
            createLog(logModel, serializer.data, logTransaccion)
            return Response(serializer.data, status=status.HTTP_200_OK)
    except Exception as e:
        err = {"error": 'Un error ha ocurrido: {}'.format(e)}
...
def productos_update_landing(request, pk):
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            serializer = ProductosLandingSerializer(query, data=request.data, partial=True)
            if serializer.is_valid():
                serializer.save()
                createLog(logModel, serializer.data, logTransaccion)
                return Response(serializer.data)
